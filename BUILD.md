...
where `INTERVAL` denotes the time in seconds before a new trace file is generated, `EXPORTER_ID` sets the exporter name, `OUTPUT` is the directory in which trace files are written, and `FILTER` is the filter expression used to filter collected events.
