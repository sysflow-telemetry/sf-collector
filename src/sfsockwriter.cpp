...
SFSocketWriter::SFSocketWriter(context::SysFlowContext *cxt, time_t start) : writer::SysFlowWriter(cxt, start), m_sock(0) {
  m_sockPath = m_cxt->getOutputFile();
}

SFSocketWriter::~SFSocketWriter() {
  close(m_sock);
}
...
int SFSocketWriter::initialize() {
  ...
  if((m_sock = socket(AF_UNIX, SOCK_SEQPACKET, 0)) < 0) {
    SF_ERROR(m_logger,
                 "Unable to create domain socket object. Error Code: "
		 << std::strerror(errno));
    exit(EXIT_FAILURE);
  }
  ...
  strncpy(addr.sun_path, m_sockPath.c_str(), sizeof(addr.sun_path)-1);
     if(connect(m_sock, (struct sockaddr*)&addr, sizeof(addr)) < 0 ) {
       SF_ERROR(m_logger,
                 "Unable to connect to domain socket: " << m_sockPath << ". Error Code: "
		 << std::strerror(errno));
       exit(EXIT_FAILURE);
     }
  writeHeader();
