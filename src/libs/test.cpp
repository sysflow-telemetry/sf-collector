...
void process_sysflow(sysflow::SFHeader* header, sysflow::Container* cont, sysflow::Process* proc, sysflow::File* f1, sysflow::File* f2, sysflow::SysFlow* flow) {
  ...
  printf("Process: PID %lu Creation Time, %lu, Exe %s, Exe Args %s, User Name %s, Group Name %s, TTY %d\n", proc->oid.hpid, proc->oid.createTS, proc->exe.c_str(), proc->exeArgs.c_str(), proc->userName.c_str(), proc->groupName.c_str(), proc->tty);
  if(f1 != nullptr) {
    printFile(f1);
  }
  if(f2 != nullptr) {
  ...
  switch(flow->rec.idx()) {
    ...
    case SF_PROC_FLOW: {
      //sysflow::ProcessFlow pf = flow->rec.get_ProcFlow();
      printf("Proc Flow received!\n");
      break;
    }
    default: {
      printf("Received an flow type not expected %lu\n", flow->rec.idx());
      break;
    }
    }
