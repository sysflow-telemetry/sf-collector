...
void ControlFlowProcessor::setUID(api::SysFlowEvent *ev) {
  m_procEvtPrcr->setUID(ev);
}

inline void ControlFlowProcessor::processFlow(api::SysFlowEvent *ev,
                                              OpFlags flag) {
  bool created = false;
...
inline void ControlFlowProcessor::processNewFlow(api::SysFlowEvent *ev,
                                                 ProcessObj *proc,
                                                 OpFlags flag) {
  auto *pf = new ProcessFlowObj();
  pf->exportTime = utils::getExportTime(m_cxt);
...
int ControlFlowProcessor::handleProcEvent(api::SysFlowEvent *ev) {
  OpFlags flag = ev->opFlag;

  switch (flag) {
  case OP_EXIT: {
    if (m_cxt->isProcessFlowEnabled() && !ev->isMainThread()) {
      processFlow(ev, flag);
    }
    if (ev->isMainThread() || !m_cxt->isProcessFlowEnabled()) {
      if (m_cxt->isProcessFlowEnabled()) {
        bool created = false;
        ProcessObj *proc =
            m_processCxt->getProcess(ev, SFObjectState::REUP, created);
        if (proc->pfo != nullptr) {
          proc->pfo->procflow.endTs = ev->getTS();
  ...
  case OP_CLONE: {
    if (!m_cxt->isProcessFlowEnabled()) {
      ...
      int res = ev->getSysCallResult();
      if ((ev->isMainThread() && res == 0) ||
          (res > 0 && !ev->isCloneThreadSet())) {
        m_procEvtPrcr->writeCloneEvent(ev);
      } else {
        processFlow(ev, flag);
      }
...
void ControlFlowProcessor::exportProcessFlow(ProcessFlowObj *pfo) {
  pfo->procflow.endTs = utils::getSystemTime(m_cxt);
  ...
  pfo->procflow.ts = utils::getSystemTime(m_cxt);
  ...
  for (auto it = m_pfSet->begin(); it != m_pfSet->end();) {
    ...
    if ((*it)->pfo->exportTime <= now) {
      ...
      if (difftime(now, (*it)->pfo->lastUpdate) >=
        ...
        ProcessObj *p = (*it);
        it = m_pfSet->erase(it);
        p->pfo->exportTime = utils::getExportTime(m_cxt);
        m_pfSet->insert(p);
      }
      i++;
    } else {
