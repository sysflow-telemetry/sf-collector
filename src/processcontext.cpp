...
ProcessObj* ProcessContext::getProcess(sinsp_evt* ev, SFObjectState state, bool& created) {
      ...
      while (mt != nullptr) {
        ...
        key.createTS = mt->m_clone_ts;
        key.hpid = mt->m_pid;
        SF_DEBUG(m_logger, "PARENT PID: " << mt->m_pid << " ts "
                                          << mt->m_clone_ts
...
bool ProcessContext::exportProcess(OID* oid) {
    ...
    bool expt = false;
    if (p == nullptr) {
      SF_ERROR(m_logger, "Can't find process to export! oid: "
                             << oid->hpid << " " << oid->createTS);
      return expt;
    }
...
void ProcessContext::printNetworkFlow(ProcessObj* proc) {
     ...
     for(NetworkFlowTable::iterator it = proc->netflows.begin(); it != proc->netflows.end(); it++) {
        cout << "Netflow: " << it->first.ip1 << " " << it->first.ip2 << " " << it->first.port1 << " " << it->first.port2 << " " << it->first.tid << " " << it->first.fd << endl;
